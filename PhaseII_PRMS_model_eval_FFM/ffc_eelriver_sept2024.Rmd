...
model_data <- read.csv("EEL_spring_summer_calibration_rev2/eel_subbasins.spring_summer_rev2_sub_cfs.csv", check.names = F)
...
for(i in 1:length(gage_file_list)) {
  ...
  # problem_gages <- c("11476500", "11475800") 
...
write.csv(export.gage, paste("FFC_outputs/EEL_spring_summer_calibration_rev2/csv_results/", Model_Watershed_eval, "_gage_", "ffc_results_", today(),".csv", sep = ""), row.names = F)
...
write.csv(export.model, paste("FFC_outputs/EEL_spring_summer_calibration_rev2/csv_results/", Model_Watershed_eval, "_model_rev2_", "ffc_results_", today(),".csv", sep = ""), row.names = F)
...
#remove gage flow metric years that were not impaired
  #for 2 gages, remove years that were not considered unimpaired (<2011)
joined_df_col <- joined_df_col[! ((joined_df_col$gageID == 11476500 | joined_df_col$gageID == 11475800) & (joined_df_col$Year<1985 | joined_df_col$Year>2000)),]
#for 1 MF dos rios ref gage, remove calibration years <=2011 to only look at ref validation period
joined_df_col <- joined_df_col[! ((joined_df_col$gageID == 11473900) & (joined_df_col$Year<2010)),]
#for 1 Lower Eel ref gage, remove calibration years <2010
joined_df_col <- joined_df_col[! ((joined_df_col$gageID == 11478500) & (joined_df_col$Year<2010)),]
...
for(p in 1:length(gage_list)){
  ...
  for (y in 1:length(unique.component)){
    ...
    if(gage.type.p == "Reference"){ #using updated Type2 column from Kris' table
      ggsave(file = paste("FFC_outputs/EEL_spring_summer_calibration_rev2/boxplots/reference_calibration/", Model_Watershed_eval,"_", gageID.p2,"_",unique(df.2$title_component),"_boxplot.jpg",sep = ""), 
             width = 180, height = 180, unit = "mm", dpi = 300, boxplot)
    }else if(gage.type.p == "Validation" & gage.validation.notes.p == "Impaired"){ 
      ggsave(file = paste("FFC_outputs/EEL_spring_summer_calibration_rev2/boxplots/validation_impaired/", 
      ...
      ggsave(file = paste("FFC_outputs/EEL_spring_summer_calibration_rev2/boxplots/validation_non-impaired/", Model_Watershed_eval,"_", gageID.p2,"_",unique(df.2$title_component),"_boxplot.jpg",sep = ""), 
...
#remove gage flow metric years that were not impaired
  #for 2 gages, remove years that were not considered unimpaired (<2011)
joined_df_scatter <- joined_df_scatter[! ((joined_df_scatter$gageID == 11476500 | joined_df_scatter$gageID == 11475800) & (joined_df_scatter$Year<1985 | joined_df_scatter$Year>2000)),]
#for 1 MF dos rios ref gage, remove calibration years <=2011 to only look at ref validation period
joined_df_scatter <- joined_df_scatter[! ((joined_df_scatter$gageID == 11473900) & (joined_df_scatter$Year<2010)),]
#for 1 Lower Eel ref gage, remove calibration years <2010
joined_df_scatter <- joined_df_scatter[! ((joined_df_scatter$gageID == 11478500) & (joined_df_scatter$Year<2010)),]
...
for(r in 1:length(gage_list)){
  ...
  for (z in 1:length(unique.component)){
    ...
    #saving scatter plots
      ...
      ggsave(file = paste("FFC_outputs/EEL_spring_summer_calibration_rev2/scatterplots/reference_calibration/", Model_Watershed_eval,"_", gageID.r,"_",unique(df.2$title_component),"_scatterplot.jpg",sep = ""), 
             width = 180, height = 180, unit = "mm", dpi = 300, scatter_plot)
    }else if(gage.type.r == "Validation" & validation.notes.r == "Impaired"){ 
      ggsave(file = paste("FFC_outputs/EEL_spring_summer_calibration_rev2/scatterplots/validation_impaired/", 
                          Model_Watershed_eval,"_", gageID.r,"_",unique(df.2$title_component),"_scatterplot.jpg",sep = ""), 
             width = 180, height = 180, unit = "mm", dpi = 300, scatter_plot)
      #else it's validation non-reference (minimally impaired) and reference
    }else{  
      ggsave(file = paste("FFC_outputs/EEL_spring_summer_calibration_rev2/scatterplots/validation_non-impaired/", Model_Watershed_eval,"_", gageID.r,"_",unique(df.2$title_component),"_scatterplot.jpg",sep = ""), 
    ...
    residual_plot <- ggplot(df.2, aes(x = gage.value, y = residual))+
      ...
      subtitle = paste0("Gage Name:", gage.name.r, ", Gage ID:", gageID.r, ", Model ID:", modelID.r))
        ...
        if(gage.type.r == "Reference"){ #using updated Type2 column from Kris' table
      ggsave(file = paste("FFC_outputs/EEL_spring_summer_calibration_rev2/residuals/reference_calibration/", Model_Watershed_eval,"_", gageID.r,"_",unique(df.2$title_component),"_residualplot.jpg",sep = ""), 
             width = 180, height = 180, unit = "mm", dpi = 300, residual_plot)
    }else if(gage.type.r == "Validation" & validation.notes.r == "Impaired"){ 
      ggsave(file = paste("FFC_outputs/EEL_spring_summer_calibration_rev2/residuals/validation_impaired/", 
                          Model_Watershed_eval,"_", gageID.r,"_",unique(df.2$title_component),"_residualplot.jpg",sep = ""), 
             width = 180, height = 180, unit = "mm", dpi = 300, residual_plot)
      #else it's validation non-reference (minimally impaired) and reference
    }else{  
      ggsave(file = paste("FFC_outputs/EEL_spring_summer_calibration_rev2/residuals/validation_non-impaired/", Model_Watershed_eval,"_", gageID.r,"_",unique(df.2$title_component),"_residualplot.jpg",sep = ""), 
