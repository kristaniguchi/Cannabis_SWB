#'  Cannabis SWB Phase III - SWB's impaired flow evaluation at the functional flow metric level
#'  
#'  This script takes FFMs calculated from gages (observed data) and evaluates model performance for every gage
#'  SWB's impaired flows were estimated by taking modeled unimpaired flows from PRMS model and subtracting monthly demand data (cumulative point of diversions) for each watershed draining to gages
...
#FFM directory where all FFMs are saved
FFM_dir <- "C:/Users/kristinet/SCCWRP/Cannabis E-Flows - General/Data/RawData/NC_impaired_flows_20250515/python_edit_SCCWRP/outputs/FFM_newFFC/"
...
#read in model data
ind.model <- grep("Impaired_FFM_NC_LOIs_ALL.csv", list.files.all)
