...
for(i in 1:length(list.files)){
  ...
  if(length(flows.upstream.sub$site_id)<1){
    ...
    flow.file <- flows.upstream.sub %>% 
      select("datetime", "unimpaired_discharge_mean") %>% 
      rename("date" = "datetime",
             "flow" = "unimpaired_discharge_mean")
...
#write csv
write.csv(outputs_ffc, file="./FFM/Unimpaired_FFM_upstream_tech_comid_gages.csv")
write.csv(warnings_ffc, file="./FFM/Unimpaired_FFM_upstream_tech_comid_gages_warnings.csv")
...
#set period of record for FFM calculations to WY 2002-2022 (POR for upstream impaired flows)
POR <- c(2002, 2022)

#FFM metric names etc
metric.names <- read.csv("C:/Users/kristinet/SCCWRP/Cannabis E-Flows - General/Data/Working/Watershed_Delineation_Tool/Modeled_Flow/Eel_River/Lookup_Tables/all_metric_def_list_FFMs_v2 1.csv")

######################################  DONT NEED TO REDO THIS - CHECK to see reference time periods??
