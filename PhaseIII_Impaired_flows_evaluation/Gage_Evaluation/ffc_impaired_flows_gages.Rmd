---
title: "ffc_redwoodcreek_nov2024"
...
#gage data in Redwood Creek
gage_data <- ("Redwood_Creek_rev2/GAGES/")
#functional flow metric names
metric_categories_file <- read.csv("Eel_River/Lookup_Tables/all_metric_def_list_FFMs_v2 1.csv")

############
#modeled PRMS outputs, need to reformat since separate calibration outputs to use
#little river has may to sept data and oct to april data
model_data1 <- read.csv("Redwood_Creek_rev2/optimization1_May_to_Sep_sub_cfs.csv", check.names = F) 
# Convert the Date column to Date format
model_data1$Date <- as.Date(model_data1$Date)
# Filter data for months between May (5) and September (9)
filtered_data <- subset(model_data1, format(Date, "%m") %in% sprintf("%02d", 5:9))
#read in second calibration dataset Oct to April

model_data2 <- read.csv("Redwood_Creek_rev2/optimization2_Oct_to_Apr_sub_cfs.csv", check.names = F) 
# Convert the Date column to Date format
model_data2$Date <- as.Date(model_data2$Date)
# Filter data for months from October to April
filtered_data2 <- subset(model_data2, format(Date, "%m") %in% c("10", "11", "12", "01", "02", "03", "04")) %>% 
  bind_rows(filtered_data) %>% 
  arrange(Date)
#save as model data
model_data <- filtered_data2

#write combined calibration dataset (final)
write.csv(model_data, file="Redwood_Creek_rev2/Redwood_Creek_rev2_calib_combined_all_cfs.csv", row.names=FALSE)
#also save to original model file location
write.csv(model_data, file="C:/Users/kristinet/SCCWRP/Cannabis E-Flows - General/Data/RawData/PRMS_Models_for_Review/REDWOOD_CREEK_recalibration_11-6-24/Redwood_creek_PRMS/Redwood_Creek_rev2_calib_combined_all_cfs.csv", row.names=FALSE)
...
dir.create("FFC_outputs/Redwood_Creek_rev2/boxplots/", recursive=TRUE)
dir.create("FFC_outputs/Redwood_Creek_rev2/csv_results/", recursive=TRUE)
dir.create("FFC_outputs/Redwood_Creek_rev2/FFM_eval/", recursive=TRUE)
dir.create("FFC_outputs/Redwood_Creek_rev2/residuals/", recursive=TRUE)
dir.create("FFC_outputs/Redwood_Creek_rev2/scatterplots/", recursive=TRUE)
#create subfolders for plots
dir.create("FFC_outputs/Redwood_Creek_rev2/boxplots/reference_calibration/", recursive=TRUE)
dir.create("FFC_outputs/Redwood_Creek_rev2/residuals/reference_calibration/", recursive=TRUE)
dir.create("FFC_outputs/Redwood_Creek_rev2/scatterplots/reference_calibration/", recursive=TRUE)
dir.create("FFC_outputs/Redwood_Creek_rev2/boxplots/validation_impaired/", recursive=TRUE)
dir.create("FFC_outputs/Redwood_Creek_rev2/residuals/validation_impaired/", recursive=TRUE)
dir.create("FFC_outputs/Redwood_Creek_rev2/scatterplots/validation_impaired/", recursive=TRUE)


write.csv(export.gage, paste("FFC_outputs/Redwood_Creek_rev2/csv_results/", Model_Watershed_eval, "_gage_", "ffc_results_", today(),".csv", sep = ""), row.names = F)
...
write.csv(export.model, paste("FFC_outputs/Redwood_Creek_rev2/csv_results/", Model_Watershed_eval, "_model_rev2_", "ffc_results_", today(),".csv", sep = ""), row.names = F)
...
joined_df_col <- joined_df_col[! ((joined_df_col$gageID == 11481500) & (joined_df_col$Year<2011)),]
...
for(p in 1:length(gage_list)){
  ...
  for (y in 1:length(unique.component)){
    ...
    if(gage.type.p == "Reference"){ #using updated Type2 column from Kris' table
      ggsave(file = paste("FFC_outputs/Redwood_Creek_rev2/boxplots/reference_calibration/", Model_Watershed_eval,"_", gageID.p2,"_",unique(df.2$title_component),"_boxplot.jpg",sep = ""), 
             width = 180, height = 180, unit = "mm", dpi = 300, boxplot)
    }else if(gage.type.p == "Validation" & gage.validation.notes.p == "Impaired"){ 
      ggsave(file = paste("FFC_outputs/Redwood_Creek_rev2/boxplots/validation_impaired/", 
      ...
      ggsave(file = paste("FFC_outputs/Redwood_Creek_rev2/boxplots/validation_non-impaired/", Model_Watershed_eval,"_", gageID.p2,"_",unique(df.2$title_component),"_boxplot.jpg",sep = ""), 
...
#for ref gage, remove calibration years <2011 to only look at ref validation period
joined_df_scatter <- joined_df_scatter[! ((joined_df_scatter$gageID == 11481500) & (joined_df_scatter$Year<2011)),]
...
for(r in 1:length(gage_list)){
  ...
  for (z in 1:length(unique.component)){
    ...
    #saving scatter plots
      ...
      ggsave(file = paste("FFC_outputs/Redwood_Creek_rev2/scatterplots/reference_calibration/", Model_Watershed_eval,"_", gageID.r,"_",unique(df.2$title_component),"_scatterplot.jpg",sep = ""), 
             width = 180, height = 180, unit = "mm", dpi = 300, scatter_plot)
    }else if(gage.type.r == "Validation" & validation.notes.r == "Impaired"){ 
      ggsave(file = paste("FFC_outputs/Redwood_Creek_rev2/scatterplots/validation_impaired/", 
                          Model_Watershed_eval,"_", gageID.r,"_",unique(df.2$title_component),"_scatterplot.jpg",sep = ""), 
             width = 180, height = 180, unit = "mm", dpi = 300, scatter_plot)
      #else it's validation non-reference (minimally impaired) and reference
    }else{  
      ggsave(file = paste("FFC_outputs/Redwood_Creek_rev2/scatterplots/validation_non-impaired/", Model_Watershed_eval,"_", gageID.r,"_",unique(df.2$title_component),"_scatterplot.jpg",sep = ""), 
    ...
    residual_plot <- ggplot(df.2, aes(x = gage.value, y = residual))+
      ...
      subtitle = paste0("Gage Name:", gage.name.r, ", Gage ID:", gageID.r, ", Model ID:", modelID.r))
        ...
        if(gage.type.r == "Reference"){ #using updated Type2 column from Kris' table
      ggsave(file = paste("FFC_outputs/Redwood_Creek_rev2/residuals/reference_calibration/", Model_Watershed_eval,"_", gageID.r,"_",unique(df.2$title_component),"_residualplot.jpg",sep = ""), 
             width = 180, height = 180, unit = "mm", dpi = 300, residual_plot)
    }else if(gage.type.r == "Validation" & validation.notes.r == "Impaired"){ 
      ggsave(file = paste("FFC_outputs/Redwood_Creek_rev2/residuals/validation_impaired/", 
                          Model_Watershed_eval,"_", gageID.r,"_",unique(df.2$title_component),"_residualplot.jpg",sep = ""), 
             width = 180, height = 180, unit = "mm", dpi = 300, residual_plot)
      #else it's validation non-reference (minimally impaired) and reference
    }else{  
      ggsave(file = paste("FFC_outputs/Redwood_Creek_rev2/residuals/validation_non-impaired/", Model_Watershed_eval,"_", gageID.r,"_",unique(df.2$title_component),"_residualplot.jpg",sep = ""), 
